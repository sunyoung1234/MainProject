...
public class MemberController {
	...
	public String registDo(HttpServletRequest req, MultipartFile img, HttpSession session) {
	    ...
	    try {
	        ...
	        if (img != null && !img.isEmpty()) {
	            ...
	            imgPath = "resources/assets/default-prof.jpg"; // 기본 이미지 경로 설정
	        }

	        int defaultMemLevel = 0; // 기본 회원 레벨 설정
	        MemberDTO member = new MemberDTO(id, pw, name, phone, email, address, imgPath, defaultMemLevel);
	        memberService.registMember(member);
	    } catch (IOException e) {
	        e.printStackTrace();
	        return "redirect:/registView"; // 에러 발생 시 회원가입 페이지로 돌아가기
	    }
	    return "redirect:/loginView"; // 가입 후 로그인 페이지로 이동
	}
	...
	public String memberUpdate(HttpServletRequest req, MultipartFile img, HttpSession session) {
	    ...
	    String address = req.getParameter("memAddress");	// 여기서 왜 null이 뜰까... why..?
	    // 제대로 데이터가 불러와지는지 체크
	    ...
	    try {
	        ...
	        int defaultMemLevel = 0; // 기본 회원 레벨 설정
	        MemberDTO updateMem = new MemberDTO(id, pw, name, phone, email, address, imgPath, defaultMemLevel);
