...
<body>
	...
	<c:if test="${bNameList.size() > 0 }"> 
		<button id="download-pdf">PDF 다운로드</button>
		<select id="selectResult">
			<c:forEach items="${bNameList }" var="bname" varStatus="status">
				<option value="${status.index }"> ${bname } </option>
			</c:forEach>
		</select>
	...
	<c:if test="${bNameList.size() == 0 }"> 
		<button style="display: none;" id="download-pdf">PDF 다운로드</button>
		<select id="selectResult" style="display:none;"> 
		</select>
	    <div style="display: none;" id="capture-div" class="container1">  
	        <h1>에너지 결과 페이지</h1>
	        <h5>건물명 : ${userB.buildingName }</h5>
	        <div class="content">
	            <div class="left-section">
	                ...
	                <div class="energy-summary">
					    <canvas id="doughnutChart3" width="200" height="200"></canvas>
					    <div class="summary-text">
					        <p><strong style="color: orange; font-size: 2em;"></strong></p>
					        <p id="savePercent" style="display:none;"></p>
